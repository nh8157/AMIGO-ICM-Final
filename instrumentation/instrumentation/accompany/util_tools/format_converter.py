    ...
    for note in chord_track.notes:
        if len(chord_set) == 0:
            ...
            if note.start == chord_time[0] and note.end == chord_time[1]:
                ...
                if len(chord_set) == 3:
                    idx = str(((chord_set[1]-chord_set[0]), (chord_set[2]-chord_set[1])))
                    quality = distr2quality.get(idx)
                elif len(chord_set) >= 4:
                    idx = str(((chord_set[1]-chord_set[0]), (chord_set[2]-chord_set[1]), (chord_set[3]-chord_set[2])))
                    quality = distr2quality.get(idx)
